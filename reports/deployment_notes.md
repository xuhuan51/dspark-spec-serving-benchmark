...
Use speculative decoding as a conditional optimization:

```text
if workload_is_domain_matched
   and expected_output_tokens is large
   and concurrency is below the measured breakpoint
   and GPU memory headroom is sufficient:
       enable speculative decoding
else:
       use baseline decoding
```
